...
public static class GridInfoCommand
{
    public static Command Create(Option<string?> sessionOption)
    {
        ...
        var command = new Command("grid-info", "Get row count, column count, and column headers from a Grid/Table element");
        ...
        command.SetAction((ParseResult parseResult) =>
        {
            ...
            try
            {
                ...
                var result = AutomationEngine.GetGridInfo(element, elementId);

                CommandHelper.RecordStep(session, "elem grid-info", elementId, null, true);

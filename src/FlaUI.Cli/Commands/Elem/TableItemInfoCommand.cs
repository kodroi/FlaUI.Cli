...
public static class TableItemInfoCommand
{
    public static Command Create(Option<string?> sessionOption)
    {
        ...
        var command = new Command("table-item-info", "Get row and column header info for a cell via the TableItem pattern");
        ...
        command.SetAction((ParseResult parseResult) =>
        {
            ...
            try
            {
                ...
                var result = AutomationEngine.GetTableItemInfo(element, elementId);

                CommandHelper.RecordStep(session, "elem table-item-info", elementId, null, true);

...
public static class GetTextCommand
{
    public static Command Create(Option<string?> sessionOption)
    {
        ...
        var command = new Command("get-text", "Get the full text content of an element via the Text pattern (RichTextBox, Document)");
        ...
        command.SetAction((ParseResult parseResult) =>
        {
            ...
            try
            {
                ...
                var result = AutomationEngine.GetText(element, elementId);

                CommandHelper.RecordStep(session, "elem get-text", elementId, null, true);

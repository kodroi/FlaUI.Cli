...
public static class GetRangeCommand
{
    public static Command Create(Option<string?> sessionOption)
    {
        ...
        var command = new Command("get-range", "Get an element's range value, minimum, maximum, and step sizes via the RangeValue pattern");
        ...
        command.SetAction((ParseResult parseResult) =>
        {
            ...
            try
            {
                ...
                var result = AutomationEngine.GetRangeValue(element, elementId);

                CommandHelper.RecordStep(session, "elem get-range", elementId, null, true);
